get555imgmatrix = (scramble) => {
  let cube = {
    up: Array.from({ length: 5 }, () => Array.from({ length: 5 }, () => "w")),
    down: Array.from({ length: 5 }, () => Array.from({ length: 5 }, () => "y")),
    left: Array.from({ length: 5 }, () => Array.from({ length: 5 }, () => "o")),
    right: Array.from({ length: 5 }, () =>
      Array.from({ length: 5 }, () => "r")
    ),
    front: Array.from({ length: 5 }, () =>
      Array.from({ length: 5 }, () => "g")
    ),
    back: Array.from({ length: 5 }, () => Array.from({ length: 5 }, () => "b")),
  };
  ...
  rotate_face_clkwise = (face) => {
    ...
    for (let index = 0; index < 5; index++) {
      face[0][index] = current_face[4 - index][0];
    }

    for (let index = 0; index < 5; index++) {
      face[index][0] = current_face[4][index];
    }

    for (let index = 0; index < 5; index++) {
      face[4][index] = current_face[4 - index][4];
    }

    for (let index = 0; index < 5; index++) {
      face[index][4] = current_face[0][index];
    }

    for (let index = 1; index < 4; index++) {
      face[1][index] = current_face[4 - index][1];
    }

    for (let index = 1; index < 4; index++) {
      face[index][1] = current_face[3][index];
    }

    for (let index = 1; index < 4; index++) {
      face[3][index] = current_face[4 - index][3];
    }

    for (let index = 1; index < 4; index++) {
      face[index][3] = current_face[1][index];
    }
